...
(function($, videojs, undefined) {
   ...
   var templateAds = {
      'image': _.template("<div key='<%=key %>' class='adSoha-reached-overlay data-adSoha-reached-overlay-<%=position %>'> <div class='adSoha-reached-overlay-image'> <img src='<%= src %>' alt='Smiley face' height='20%'' width='20%''> </img> </div> </div>"),
      CSS: {
         'image': _.template(".adSoha-reached-overlay{visibility:visible;position:absolute;z-index:100000;top:0}.data-adSoha-reached-overlay-bottom-left{bottom:10px;left:10px}.data-adSoha-reached-overlay-bottom-right{bottom:10px;right:10px}.data-adSoha-reached-overlay-top-left{top:10px;left:10px}.data-adSoha-reached-overlay-top-right{top:10px;right:10px}")
      }
   }
   ...
   function onMarkerReached(marker) {
      ...
      if (marker.options.src && marker.type && marker.isDisplayed == false) {
        switch (marker.type) {
          case 'image': {
            var breakOverlay = $(templateAds.image(marker.options));
            videoWrapper.append(breakOverlay);
            if (marker.options.displayTime) {
               setTimeout(function() {
                  videoWrapper.find('.adSoha-reached-overlay[key="' + marker.key + '"]').remove();
               }, marker.options.displayTime);
            }
            break;
          }
          case 'html': {
            //var breakOverlay = $(templateAds.image({src: marker.src}));
          }
        }
   ...
   function registerVideoJsMarkersPlugin(options) {
      ...
      var setting      = $.extend(true, {}, defaultSetting, options),
          ...
          overlayIndex = -1;
      // TuanLDT add -S
      var style = $('<style class="adSoha-style"></style>').html(templateAds.CSS.image)[0];
      ...
      function onTimeUpdate() {
         ...
         */
         ...
         if (newMarkerIndex != currentMarkerIndex) {
            // trigger event
            if (newMarkerIndex != -1 && setting.onMarkerReached) {
              setting.onMarkerReached.call(this,markersList[newMarkerIndex]);
            }
      ...
      player.on("loadedmetadata", function() {
         if (player.markers.contentSrc == player.currentSrc()) {
            initialize();
         } else {
            player.markers.removeAll();
            //breakOverlay.remove();
